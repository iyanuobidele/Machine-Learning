...
class SVM(object):
    ...
    def experiments(self, one=True, two=True, three=True):
        ...
        with open(r'C:\Users\Iyanu\Desktop\Python\ML\data\3\norm_spam_train.csv','r') as dest_f:
        ...
        with open(r'C:\Users\Iyanu\Desktop\Python\ML\data\3\norm_spam_test.csv','r') as tdest_f:
        ...
        c_array = np.linspace(start=0.1, stop=1, num=10)    # Possible C values to test with. Evenly spaced 20 values
        temp_array = []
        avg_c_acc = []
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            # This returns the decision values of the test set based on the SVM Model
        ...
        if one:
            ...
            accuracy, precision, recall, fpr = self.precision_recall_calc(scores, self.train[:, 57:58], 0)
            print "Using C = " + str(c_array[index]) + "\n"
            ...
            tpr_array = []
            fpr_array = []
            # Generate evenly spaced numbers between -1 amd 1
            even_thresh = np.linspace(start=-1, stop=1, num=200)
            for ii in range(len(even_thresh)):
                _, _, t, f = self.precision_recall_calc(scores, self.train[:, 57:58], even_thresh[ii])
                tpr_array.append(t)
                fpr_array.append(f)
            ...
            plt.axis([0, 1, 0, 1])
            plt.plot(fpr_array, tpr_array, 'ro')
        ...
        if two:
            ...
            plt.ylim(0,1.0)
            plt.plot(range(2,58), acc_array, 'ro')
            plt.title("Accuracy vs No of features with highest weights")
            plt.show()
